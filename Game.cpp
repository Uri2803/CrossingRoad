	...
	while (playing)
	{
		...
		else if (state == editSave)
		{
			...
			if (key == '\r')
			{
				...
				else if (counter == 3)
				{
					...
					while (!file.eof())
					{
						file >> _x >> _y >> _dir >> _velocity >> _type;
						if (_type == "Car")
						{
							if (_dir == 1)
		...
		else if (state == newGame)
		{
			if (key == '\r' && !name.empty())
			{
				...
				scene.join();
				scene = thread(&Game::drawGame, this);
				// maybe load the level of the game here, include bla bla tree, traffic light, car, animal number... or something better than that
				// should have been the back end
				for (auto i : vehicleList)
					delete i;
				vehicleList.clear();
				for (auto j : animalList)
					delete j;
				animalList.clear();
				vehicleList.push_back(new Car(50, 16, right_, 3));
				// movingObjList.push_back(temp);
				vehicleList.push_back(new Truck(26, 3, left_, 2));
				vehicleList.push_back(new Car(30, 26, left_, 1));
				vehicleList.push_back(new Truck(10, 33, right_, 4));
		...
		else if (state == pauseMenu)
		{
			...
			if (key == '\r')
			{
				...
				if (counter == 2)
				{
					...
					for (auto i : animalList)
						file << " " << i->x << " " << i->y << " " << i->getDir() << " " << i->velocity() << " " << i->getName();
					for (auto i : vehicleList)
						file << " " << i->x << " " << i->y << " " << i->getDir() << " " << i->velocity() << " " << i->getName();
					file.close();
					// saveFile.push_back(name);
...
void Game::drawGame()
{
	...
	while (state == playGame)
	{
		...
		for (auto i : vehicleList)
		{
			if (player->isImpact(i))
			{
				drawPicture(player->x, player->y, car1, bgColor | FG_DARK_RED);
				playHitSound();
